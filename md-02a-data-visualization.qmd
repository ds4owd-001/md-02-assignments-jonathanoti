...
5.  What are the data types of the three variables used for aesthetic mappings? **Numerical and non-numerical**
...
The code for removing the outliers from the boxplot was found here: <https://stackoverflow.com/questions/5677885/ignore-outliers-in-ggplot2-boxplot>
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: **The two functions are for categorical data, however, geom_col() only works with defined x and y data coordinates where as the geom_bar() works with the raw data and automatically groups the data or counts the frequency once the categories are defined.**
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function. **geom_histogram() is for representing coninuous data whiles the geom_bar is used for representing categorical data.**

7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: **The seq() function is for creating the bins for the histogram. The limits are 40 and 85 with interval of 2.5. Changing the values will alter the number of bins and the interval. This will also affect the chart as well.**
